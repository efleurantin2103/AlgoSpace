...
fprintf('Creating animated GIF...\n');
fprintf('Working directory: %s\n', pwd);

for h = 1:length(mu)
    fprintf('Processing frame %d/%d, mu = %.4f\n', h, length(mu), mu(h));
    ...
    figure('Visible', 'off') % Don't show figures during creation
    ...
    title(sprintf('Unstable Manifolds'), 'FontSize', 14, 'Interpreter', 'latex')
    hold off
    ...
    title(sprintf('Center Manifold'), 'FontSize', 14, 'Interpreter', 'latex')
    hold off

    % Add parameter display
    sgtitle(sprintf('$\\mu = %.4f$, $\\gamma = %.4f$', mu(h), gamma), ...
    ...
    % Capture frame for GIF (MUST be inside the loop, before closing figure)
    frame = getframe(gcf);
    im = frame2im(frame);
    ...
    % Write to GIF file
    if h == 1
        imwrite(imind,cm,'jump_animation.gif','gif', 'Loopcount',inf,'DelayTime',0.8);
    else
        imwrite(imind,cm,'jump_animation.gif','gif','WriteMode','append','DelayTime',0.8);
    end
